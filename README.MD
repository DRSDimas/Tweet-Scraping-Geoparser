...
This repository provides two Python scripts (`scraper.py` and `geoparser.py`) to map the online outcry surrounding a real-world event. For example, I'm using it for mapping the digital footprint of a mass demonstration, standoff, arson, looting, and general chaos surrounding the 29th-30th August 2025 Protest in Jakarta, Indonesia.

![Heatmap Example](./Jakarta%20Heatmap.png)
...
## Important Disclaimer
  * This tool only performs a **CONFIRMATORY ANALYSIS, NOT AN EXPLORATORY ONE**. The `geoparser.py` script is not an advanced AI that can automatically detect and discover new, unknown locations from tweet text (I can't make it reliable enough yet, probably you can help me the future). It can only find locations that you have pre-defined in its (`DEFINITIVE_LOCATIONS`) dictionary. 
